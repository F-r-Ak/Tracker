...
export class GovernateGeoPointsService extends HttpService {
  protected get baseUrl(): string {
    return 'v1/governorategeopoints/';
  }

  getGovernateGeoPoint(id: string) {
    return this.get<TestDto>({ apiName: `Get/${id}` });
  }

  getEditGovernateGeoPoint(id: string) {
    return this.get<TestDto>({ apiName: `getEdit/${id}` });
  }

  get governateGeoPoints() {
