...
export abstract class BaseListComponent extends BaseComponent implements OnInit {
  ...
  handleEvent(dataTableEvent: any): void {
    ...
    if (dataTableEvent.eventType == 'delete') {
      console.log('dataTableEvent at delete: ', dataTableEvent.data);
      this.deleteData(dataTableEvent.data);
    }
    if (dataTableEvent.eventType == 'deleteRange') {
      console.log('dataTableEvent at delete range:', dataTableEvent.data);
  ...
  applyFilter(value: string, column: string): void {
    ...
    this.resetOpt();
    this.dataTableService.opt.filter[column] = value; // Set the filter
    this.loadDataFromServer(); // Reload data based on the filter
    console.log('enter applyFilter value:', value, 'column:', column);
  }

  search(filterArray: any): void {
    // debugger
    this.dataTableService.opt.filter = filterArray;
    console.log("this.dataTableService.opt.filter: ", this.dataTableService.opt.filter);
    this.loadDataFromServer(); // Reload data based on the filter
  }

  openDialog(component: any, pageTitle: any, data: any, closable: boolean = true): void { // Add closable parameter with default value
  ...
  loadDataFromServer(): void {
    this.dataTableService.loadData(this.tableOptions.inputUrl.getAll).subscribe({
      next: res => {
        console.log("dataTableServiceeeeeeeeeeeee: ",res.data);
  ...
  filter(value?: any, column?: any, filterColumnName?: string, dataType?: string): void {
    // console.log("value:",value + "column:",column + "filterColumnName:",filterColumnName + "dataType:",dataType);
    this.resetOpt();
