...
import { NamedHttpStatus } from '../../enums';
      ...
      if (!Number.isNaN(Number(event.status))) {
        if (event.status.toString().startsWith('2')) {
          this.alertService.success(
            event.message ? ('VALIDATION.' + event.message) : 'Successfully Done...'
          );
        } else {
          this.alertService.error(event.message ? ('VALIDATION.' + event.message) : '!NOT HANDLED ERROR!');
        }
      } else {
        ...
        switch (status) {
          case NamedHttpStatus.Created: {
            this.alertService.success(
              event.message ? 'تم بنجاح' : 'Successfully Done...'
            );
            break;
          }
